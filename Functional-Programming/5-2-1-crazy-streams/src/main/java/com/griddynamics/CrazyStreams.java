...
import com.griddynamics.exceptions.EntityNotFoundException;
import com.griddynamics.util.Account;
import com.griddynamics.exceptions.ExerciseNotCompletedException;
import com.griddynamics.util.Sex;
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> {
                    int index = account.getEmail().indexOf("@");
                    return account.getEmail().substring(index + 1);
                }));
    }
