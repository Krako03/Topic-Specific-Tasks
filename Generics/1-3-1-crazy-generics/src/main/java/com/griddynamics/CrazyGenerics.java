...
import com.griddynamics.util.BaseEntity;
import com.griddynamics.util.ExerciseNotCompletedException;
import lombok.Data;
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().findAny()
                    .filter(baseEntity -> baseEntity.getUuid() == null)
                    .isPresent();
        }
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<? extends BaseEntity> entities, T targetEntity) {
            ...
            long count = entities.stream()
                    .filter(entity -> entity.getUuid().equals(targetUuid))
                    .count();
            return count > 1;
        }
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            ...
            LocalDateTime ldt = LocalDateTime.now().minusDays(1000);
            T mostRecent = null;

            for (T element : entities) {
                if (element.getCreatedOn().isAfter(ldt)) {
                    ldt = element.getCreatedOn();
                    mostRecent = element;
                }
            }
